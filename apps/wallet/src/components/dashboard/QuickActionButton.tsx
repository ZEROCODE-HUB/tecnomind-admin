...
const QuickActionButton = ({ icon: Icon, label, isPrimary = false, onClick }: QuickActionButtonProps) => {
  return (
    <button
      onClick={onClick}
      className="col-span-1 flex flex-col items-center gap-2 group min-w-[44px] min-h-[44px]"
    >
      <div
        className={`size-16 rounded-2xl flex items-center justify-center transition-transform active:scale-95 ${
          isPrimary
